...
p2.daemon = True
p2.start()
fontText = ImageFont.truetype(font='arial.ttf', size=10, encoding='utf-8')
        ...
        if detections is not None and tImg is not None:
            ...
            for i in range(min(len(listLabels), len(listFaces), len(listTMaxs))):
                ...
                if label != 'người lạ' and label not in labels:
                    ...
                    saveDataQueue.put({"accountId": studentId, "image": "Images/615bfa7026d20336a50a7112.jpg", "checkIn": nowTime, "isSent": 0})
